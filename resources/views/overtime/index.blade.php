...
<div class="space-y-6">
    <header class="rounded-2xl bg-white p-4 shadow-soft ring-1 ring-slate-100">
        <div>
            <h1 class="text-lg font-semibold">Horas extras</h1>
            <p class="text-sm text-slate-500">Control de horas extras y domingo/festivos por tienda y mes.</p>
        </div>
    </header>

    <div class="rounded-2xl bg-white p-4 shadow-soft ring-1 ring-slate-100">
        <form method="GET" action="{{ route('overtime.index') }}" class="flex flex-wrap items-end gap-4 mb-4">
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach($stores as $s)
                        <tr class="hover:bg-slate-50">
                            <td class="px-3 py-2">
                                <a href="{{ route('overtime.store-months', ['store' => $s, 'year' => $year]) }}" class="font-semibold text-slate-900 hover:text-brand-600">{{ $s->name }}</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
