    ...
    <header class="rounded-2xl bg-white p-4 shadow-soft ring-1 ring-slate-100">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500">{{ $employee->full_name }} — {{ $overtimeRecord->date->format('d/m/Y') }} — {{ $overtimeRecord->overtimeType->name ?? '' }}</p>
            </div>
    ...
    <div class="rounded-2xl bg-white p-6 shadow-soft ring-1 ring-slate-100 max-w-md">
        <form method="POST" action="{{ route('overtime.records.update', $overtimeRecord) }}" class="space-y-4">
            ...
            <label class="block">
                ...
                <select name="overtime_type_id" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4">
                    @foreach($types as $type)
                        <option value="{{ $type->id }}" {{ old('overtime_type_id', $overtimeRecord->overtime_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                    @endforeach
                </select>
            </label>
            <label class="block">
                <span class="text-xs font-semibold text-slate-700">Horas</span>
                <input type="number" name="hours" value="{{ old('hours', $overtimeRecord->hours) }}" step="0.01" min="0" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                @error('hours')<span class="text-xs text-rose-600">{{ $message }}</span>@enderror
            </label>
