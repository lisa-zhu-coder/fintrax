...
<div class="widget-content">
    <div class="flex flex-wrap gap-3">
        ...
        @endif
        @if($canCashDeposit)
        <button type="button" onclick="document.getElementById('modalIngresarDinero').classList.remove('hidden')" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
        ...
        @endif
        @if($canCreateOrder)
        <a href="{{ route('orders.create') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
