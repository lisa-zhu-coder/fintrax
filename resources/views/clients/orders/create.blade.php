...
@section('title', 'Nuevo pedido cliente - ' . $store->name)
...
<div class="space-y-6">
    <header class="rounded-2xl bg-white p-4 shadow-soft ring-1 ring-slate-100">
        <div>
            ...
            <h1 class="text-lg font-semibold">Nuevo pedido de cliente</h1>
            <p class="text-sm text-slate-500">Tienda: {{ $store->name }}</p>
        </div>
    </header>

    <div class="rounded-2xl bg-white p-6 shadow-soft ring-1 ring-slate-100">
        <form method="POST" action="{{ route('clients.orders.store.post', $store) }}" class="space-y-4">
            @csrf
