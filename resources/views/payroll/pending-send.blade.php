...
<div class="mx-auto max-w-5xl space-y-6">
    ...
    <div class="flex flex-wrap items-center gap-3">
        <form method="POST" action="{{ route('payroll.cancel-pending') }}" id="formCancelPending" class="inline" onsubmit="return confirm('¿Cancelar? Se eliminarán las nóminas subidas y no se guardará nada.');">
            @csrf
            @if(!empty($token ?? null))
            <input type="hidden" name="payroll_token" value="{{ $token }}">
            @endif
