...
<div class="space-y-6">
    <header class="rounded-2xl bg-white p-4 shadow-soft ring-1 ring-slate-100">
        @include('partials.treasury-section-tabs', ['group' => 'cash'])
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-lg font-semibold">Editar Gasto</h1>
    ...
    <div class="rounded-2xl bg-white p-6 shadow-soft ring-1 ring-slate-100">
        <form method="POST" action="{{ route('cash-wallets.expenses.update', ['cashWallet' => $cashWallet->id, 'expense' => $expense->id]) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <label class="block">
                    ...
                    <input type="date" name="date" value="{{ old('date', $expense->financialEntry->date->format('Y-m-d')) }}" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                    @error('date')
                ...
                <label class="block">
                    ...
                    <select name="store_id" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4">
                        ...
                        @foreach($stores as $store)
                            <option value="{{ $store->id }}" {{ old('store_id', $expense->financialEntry->store_id) == $store->id ? 'selected' : '' }}>
                                {{ $store->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <label class="block">
                    ...
                    <select name="supplier_id" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4">
                        ...
                        @foreach($suppliers ?? [] as $supplier)
                            <option value="{{ $supplier->id }}" {{ old('supplier_id', $expense->financialEntry->supplier_id) == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>
                        @endforeach
                    </select>
                </label>
                ...
                <label class="block">
                    ...
                    <input type="number" name="amount" step="0.01" min="0.01" value="{{ old('amount', $expense->amount) }}" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                    @error('amount')
                        <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                    @enderror
                </label>
