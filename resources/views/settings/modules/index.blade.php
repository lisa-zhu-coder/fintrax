...
<div class="space-y-6">
    ...
    <div class="rounded-2xl bg-white p-6 shadow-soft ring-1 ring-slate-100">
        <form method="POST" action="{{ route('module-settings.update') }}" class="space-y-6">
            ...
            <div class="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-4">
                <div>
                    <p class="font-semibold text-slate-800">Activar inventario de anillos</p>
                    <p class="text-sm text-slate-500">Muestra el menú Inventario de anillos (registros por tienda y mes) en el lateral para los usuarios con permiso.</p>
                </div>
                <label class="relative inline-flex cursor-pointer items-center">
                    <input type="hidden" name="rings_inventory_enabled" value="0">
                    <input type="checkbox" name="rings_inventory_enabled" value="1" {{ old('rings_inventory_enabled', $company->rings_inventory_enabled) ? 'checked' : '' }}
                        class="peer sr-only">
                    <div class="peer h-6 w-11 rounded-full bg-slate-200 after:absolute after:left-[2px] after:top-[2px] after:h-5 after:w-5 after:rounded-full after:border after:border-slate-300 after:bg-white after:transition-all after:content-[''] peer-checked:bg-brand-600 peer-checked:after:translate-x-full peer-checked:after:border-white peer-focus:ring-4 peer-focus:ring-brand-200"></div>
                    <span class="ml-3 text-sm font-medium text-slate-700">{{ $company->rings_inventory_enabled ? 'Activado' : 'Desactivado' }}</span>
                </label>
            </div>
