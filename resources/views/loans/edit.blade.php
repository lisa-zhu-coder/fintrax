...
@section('title', 'Editar préstamo: ' . $loan->name)
...
<div class="space-y-6">
    <header class="rounded-2xl bg-white p-4 shadow-soft ring-1 ring-slate-100">
        <div class="flex items-center justify-between">
            <div>
                ...
                <h1 class="text-lg font-semibold">Editar préstamo</h1>
            </div>
    ...
    <div class="rounded-2xl bg-white p-6 shadow-soft ring-1 ring-slate-100">
        ...
        @endif
        <form method="POST" action="{{ route('loans.update', $loan) }}" class="space-y-6" id="loanForm">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <label class="block">
                    <span class="text-xs font-semibold text-slate-700">Nombre *</span>
                    <input type="text" name="name" value="{{ old('name', $loan->name) }}" required maxlength="255" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                </label>
                <label class="block">
                    ...
                    <select name="loan_type_id" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4">
                        @foreach($loanTypes as $type)
                            <option value="{{ $type->id }}" {{ old('loan_type_id', $loan->loan_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                        @endforeach
                    </select>
                </label>
                <label class="block">
                    ...
                    <select name="direction" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4">
                        <option value="company_owes" {{ old('direction', $loan->direction) === 'company_owes' ? 'selected' : '' }}>La empresa debe</option>
                        <option value="company_lends" {{ old('direction', $loan->direction) === 'company_lends' || old('direction', $loan->direction) === 'they_owe_us' ? 'selected' : '' }}>La empresa presta (nos deben)</option>
                    </select>
                </label>
                <label class="block">
                    <span class="text-xs font-semibold text-slate-700">Capital / Principal (€) *</span>
                    <input type="number" name="principal_amount" step="0.01" min="0" value="{{ old('principal_amount', $loan->principal_amount) }}" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                </label>
                <label class="block">
                    ...
                    <select name="interest_type" id="interest_type" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4">
                        <option value="">—</option>
                        <option value="fixed" {{ old('interest_type', $loan->interest_type) === 'fixed' ? 'selected' : '' }}>Fijo</option>
                        <option value="variable" {{ old('interest_type', $loan->interest_type) === 'variable' ? 'selected' : '' }}>Variable (índice + diferencial)</option>
                    </select>
                </label>
                <label class="block" id="wrap_interest_rate">
                    <span class="text-xs font-semibold text-slate-700">Tasa interés anual (%)</span>
                    <input type="number" name="interest_rate" step="0.01" min="0" max="100" value="{{ old('interest_rate', $loan->interest_rate) }}" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                </label>
                <label class="block hidden" id="wrap_interest_index">
                    ...
                    <select name="interest_index" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4">
                        ...
                        @foreach($interestIndexOptions as $value => $label)
                            <option value="{{ $value }}" {{ old('interest_index', $loan->interest_index) === $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </label>
                <label class="block hidden" id="wrap_interest_spread">
                    <span class="text-xs font-semibold text-slate-700">Diferencial (%)</span>
                    <input type="number" name="interest_spread" step="0.01" min="0" value="{{ old('interest_spread', $loan->interest_spread) }}" placeholder="ej. 1.2" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                </label>
                <label class="block hidden" id="wrap_interest_floor">
                    <span class="text-xs font-semibold text-slate-700">Suelo (%)</span>
                    <input type="number" name="interest_floor" step="0.01" min="0" value="{{ old('interest_floor', $loan->interest_floor) }}" placeholder="ej. 0.5" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                </label>
                <label class="block hidden" id="wrap_interest_cap">
                    <span class="text-xs font-semibold text-slate-700">Techo (%)</span>
                    <input type="number" name="interest_cap" step="0.01" min="0" value="{{ old('interest_cap', $loan->interest_cap) }}" placeholder="opcional" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                </label>
                <label class="block hidden" id="wrap_initial_index_rate">
                    <span class="text-xs font-semibold text-slate-700">Tasa índice para recalcular (%)</span>
                    <input type="number" name="initial_index_rate" step="0.01" min="0" value="{{ old('initial_index_rate') }}" placeholder="Opcional: recalcula tabla de amortización" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                </label>
                <label class="block">
                    <span class="text-xs font-semibold text-slate-700">Plazo (meses)</span>
                    <input type="number" name="term_months" min="1" value="{{ old('term_months', $loan->term_months) }}" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                </label>
                <label class="block">
                    <span class="text-xs font-semibold text-slate-700">Fecha inicio</span>
                    <input type="date" name="start_date" value="{{ old('start_date', $loan->start_date?->format('Y-m-d')) }}" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                </label>
                <label class="block">
                    <span class="text-xs font-semibold text-slate-700">Comisión apertura (€)</span>
                    <input type="number" name="opening_fee" step="0.01" min="0" value="{{ old('opening_fee', $loan->opening_fee) }}" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                </label>
            </div>
