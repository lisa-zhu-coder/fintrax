        ...
        <form method="POST" action="{{ route('financial.update', $entry->id) }}" class="space-y-6" id="financialForm" novalidate>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <label class="block">
                    ...
                    <div class="mt-1 text-xs text-slate-500">Mes a efectos de beneficio</div>
                </label>
                @endif
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <label class="block">
                            ...
                            <select name="income_payment_method" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4">
                                <option value="">Selecciona…</option>
                                <option value="cash" {{ old('income_payment_method', $entry->expense_payment_method) === 'cash' ? 'selected' : '' }}>Efectivo</option>
                                <option value="bank" {{ old('income_payment_method', $entry->expense_payment_method) === 'bank' ? 'selected' : '' }}>Banco</option>
                                <option value="card" {{ old('income_payment_method', $entry->expense_payment_method) === 'card' ? 'selected' : '' }}>Tarjeta</option>
                                <option value="datafono" {{ old('income_payment_method', $entry->expense_payment_method) === 'datafono' ? 'selected' : '' }}>Datáfono</option>
                            </select>
                        </label>
                    </div>
                </div>
