...
<div class="space-y-6">
    <header class="rounded-2xl bg-white p-4 shadow-soft ring-1 ring-slate-100">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-lg font-semibold">Editar Rol: {{ $role->name }}</h1>
                <p class="text-sm text-slate-500">Permisos por módulo y acción (modulo.submodulo.accion)</p>
                @if(session('company_id'))
                <p class="mt-1 text-xs text-amber-600">Los permisos se guardan solo para la empresa actual y no afectan a otras empresas.</p>
                @endif
            </div>
    ...
    <div class="rounded-2xl bg-white p-6 shadow-soft ring-1 ring-slate-100">
        <form method="POST" action="{{ route('roles.update', $role) }}" id="role-form" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <label class="block">
                    ...
                    <input type="text" name="name" value="{{ old('name', $role->name) }}" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                </label>
                <label class="block">
                    <span class="text-xs font-semibold text-slate-700">Descripción</span>
                    <input type="text" name="description" value="{{ old('description', $role->description) }}" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                </label>
            </div>

            @php
                $currentPermissions = old('permissions', $effectivePermissions ?? $role->permissions ?? []);
            @endphp
            ...
            <div class="flex items-center justify-end gap-2 border-t border-slate-200 pt-4">
                @if(session('company_id'))
                <form method="POST" action="{{ route('roles.reset-permissions', $role) }}" class="inline" onsubmit="return confirm('¿Restaurar los permisos por defecto del rol? Se eliminará la personalización de esta empresa.');">
                    @csrf
                    <button type="submit" class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
