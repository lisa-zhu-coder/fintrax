        ...
        <form method="POST" action="{{ route('ring-inventories.update', $ringInventory) }}" class="space-y-6">
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                <label class="block">
                    ...
                    @else
                        <input type="number" min="0" value="{{ old('initial_quantity', $ringInventory->initial_quantity) }}" readonly class="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-500" title="Se calcula automáticamente. Sin permiso para editar inicial."/>
                        <p class="mt-0.5 text-xs text-slate-500">Se calcula automáticamente (sin permiso para editar)</p>
                    @endif
                </label>
                <label class="block">
