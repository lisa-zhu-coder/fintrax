...
<div class="space-y-6">
    <header class="rounded-2xl bg-white p-4 shadow-soft ring-1 ring-slate-100">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-lg font-semibold">Añadir Pedido</h1>
                <p class="text-sm text-slate-500">Completa todos los datos del nuevo pedido</p>
            </div>
        ...
        <form method="POST" action="{{ route('orders.store') }}" class="space-y-6" id="orderForm">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <label class="block">
                    ...
                    <select name="supplier_id" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4">
                        ...
                        @foreach($suppliers ?? [] as $supplier)
                            <option value="{{ $supplier->id }}" {{ old('supplier_id', request('supplier_id')) == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>
                        @endforeach
                    </select>
                </label>
                ...
                <label class="block">
                    ...
                    <input type="text" name="order_number" value="{{ old('order_number') }}" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4" placeholder="Ej: PED-2025-001"/>
                </label>
                ...
                <label class="block">
                    ...
                    <select name="concept" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4">
                        ...
                        <option value="pedido" {{ old('concept') === 'pedido' ? 'selected' : '' }}>Pedido</option>
                        <option value="royalty" {{ old('concept') === 'royalty' ? 'selected' : '' }}>Royalty</option>
                        <option value="rectificacion" {{ old('concept') === 'rectificacion' ? 'selected' : '' }}>Rectificación</option>
                        <option value="tara" {{ old('concept') === 'tara' ? 'selected' : '' }}>Tara</option>
                    </select>
                </label>

                <label class="block">
                    <span class="text-xs font-semibold text-slate-700">Importe (€) *</span>
                    <input type="number" name="amount" id="orderAmount" step="0.01" min="0" value="{{ old('amount') }}" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-200 focus:ring-4"/>
                </label>
            </div>
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-4 ring-1 ring-slate-100">
                <label class="flex items-center gap-2 mb-4">
                    <input type="checkbox" id="orderSplitStores" class="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-2 focus:ring-brand-500"/>
                    <span class="text-sm font-semibold text-slate-700">Dividir este pedido entre tiendas</span>
                </label>
                <div id="orderSplitContainer" class="hidden space-y-3">
                    <div class="rounded-lg border border-slate-200 bg-slate-50 p-3">
                        ...
                        <div id="orderSplitStoreCheckboxes" class="space-y-2">
                            ...
                            <label class="flex items-center gap-2">
                                <input type="checkbox" name="order_split_stores[]" value="{{ $store->id }}" class="order-split-store-checkbox h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-2 focus:ring-brand-500"/>
...
<script>
let paymentIndex = 0;
const cashWalletsForPayments = @json($cashWallets ?? []);
const storesForPayments = @json($stores ?? []);
        ...
        return `
            <div class="flex items-center gap-3 rounded-lg border border-slate-200 bg-white p-3">
                ...
                <div class="w-20 text-right text-xs font-semibold text-slate-600">
                    <span class="order-split-percentage">${((amountPerStore / totalAmount) * 100).toFixed(1)}%</span>
                </div>
            </div>
        `;
    }).join('');
